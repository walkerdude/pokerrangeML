...
class PokerRangeClassifier:
    ...
    def predict_single_hand(self, features: np.ndarray) -> Dict[str, Any]:
        """Predict classification for a single hand"""
        prediction, probabilities = self.predict_range(features.reshape(1, -1))
        
        result = {
            'predicted_class': self.class_names[prediction[0]],
            'class_id': prediction[0],
            'probabilities': {
                self.class_names[i]: prob for i, prob in enumerate(probabilities[0])
            },
